...
An `implementation_failed` intervention remains blocked by default. Bare execute may reclassify it only when the failed run, handoff, exact HEAD, actual branch, unique incomplete owner, and nonempty owner-bound scope agree; the only tracked change is one unstaged singular Approved `tasks.md`; the supported publication detector proves every changed byte is a selected `Files` to `File(s)` label repair and current detection converges; no implementation path is dirty; and no lock or prior record exists. Untracked workflow evidence must satisfy its closed structural ownership contract. A directory name, ignored status, artifact basename, handoff summary, plugin update, or arbitrary untracked file is never recovery authority. Bare execute repeats the proof under its lease, consumes one durable allowance, retains the original handoff in recovery history, and dispatches only the implement remediation. Explicit issue queues and optional flags do not grant this recovery.
