...
`/sdlc-execute` resumes from the current issue branch and live Git/GitHub evidence. Runtime files such as `.omp/sdlc/run.json`, recovery ledgers, and old handoffs never select a stage, grant publication, or veto work; the controller lease is only a live mutex. Failed or non-green verification returns to implementation with its evidence and no fixed attempt ceiling, while an unchanged failure calls for different diagnosis rather than replaying the same smoke or push. Only a complete passing registered steering gate at the exact source head permits PR ready or merge, and cancellation closes only panes the invocation owns.
