...
Verification may own issue, spec, branch, PR, comment, check, merge, and closure mutations only through normal nmg-sdlc workflow ownership (`scripts/sdlc-execute.mjs run` and its workers), against exactly `Nunley-Media-Group/nmg-sdlc-smoke` and an invocation-scoped explicit issue queue. The queue is supplied at verify time through `NMG_SDLC_SMOKE_ISSUES` (fresh approved smoke issues provisioned through that repository's normal `/sdlc-draft-issue` and `/sdlc-write-spec`). Delivered issues are terminal and must not be reused. The nmg-sdlc verify worker is an invoker/observer only.
...
**And** a `passed` result includes nonempty evidence that every queued issue reached exact-head merge and GitHub `CLOSED` on this invocation, including issue URL, a new PR URL outside the pre-run closing-PR baseline, observed pre-merge head SHA, `MERGED`, and `CLOSED`
**And** `/sdlc-status --json` output and pre-existing merged PRs are not sufficient to pass

### AC2: Explicit multi-issue queue is executed without inventing identities

**Given** `NMG_SDLC_SMOKE_ISSUES` holds two or more unique positive issue numbers for `Nunley-Media-Group/nmg-sdlc-smoke`
**When** the gate runs
**Then** execute processes that entire queue in the configured order in one controller-owned run
**And** the gate does not draft issues, guess lowest-numbered issues, use the empty-execute picker, hard-code a reusable delivered issue, or substitute any identity that was not in the env queue
**And** missing, empty, or non-explicit `NMG_SDLC_SMOKE_ISSUES` (or invalid `config.issues` / `issuesEnv`) yields a required `failed` result
...
|----|-------------|----------|-------|
| FR1 | Keep `repository.nmg-sdlc-smoke` an always-required steering validation, but change its pass contract from status JSON to invocation-scoped delivered/merged/closed proof for the explicit queue. | Must | |
| FR2 | Allowlist exactly `Nunley-Media-Group/nmg-sdlc-smoke`. Production config is `{ "issuesEnv": "NMG_SDLC_SMOKE_ISSUES" }`. Parse that env as unique positive safe integers in configured order. Fail closed on any other remote or on missing/empty/non-explicit identities. | Must | Never hard-code a reusable delivered issue. |
...
| FR4 | Fail closed without authenticated GitHub access, valid Herdr OMP context, a clean clone, approved-spec/delivery proof, or a fresh env queue; do not skip, pass, or fixture-substitute. | Must | Execute exit 0 with `Run /sdlc-write-spec` is not pass. Historical merged PRs are not pass. |
| FR5 | Define one controller ownership boundary: provider/verify worker invokes or observes one smoke-clone execute controller; no nested nmg-sdlc execute on the plugin checkout; no re-entry of this validation. | Must | Env `NMG_SDLC_SMOKE_OWNED=1` on the child only. |
| FR6 | Never invent arbitrary issues, never reuse a delivered smoke issue, never kill foreign processes, never delete remote GitHub evidence, and never weaken exact-head delivery. | Must | |
| FR7 | Remove the local clone after pass; retain local failure evidence; leave GitHub artifacts auditable. | Must | |
| FR8 | Add deterministic regressions covering missing/invalid env, fresh ordered multi-issue argv, non-reuse vs baseline, exact current-run proof, preflight, cleanup, and stack independence. | Must | Jest fakes GitHub/Herdr/execute. |
| FR9 | Update product and tech steering (including the `NMG_SDLC_SMOKE_ISSUES` env row) so the gate is no longer described as read-only status smoke. | Must | Exact strings in design.md. README has no smoke pass wording. |
...
- Changing exact-head merge, required-check, or issue-closure semantics in `scripts/sdlc-execute.mjs` / `scripts/sdlc-deliver.mjs` beyond writing `.omp/sdlc/smoke-deliveries/<issue>.json` before merge when `NMG_SDLC_SMOKE_OWNED=1`
- Auto-drafting or auto-specifying smoke issues when the env queue is empty
...
- Editing README (no current smoke pass wording); steering snippets are the project docs for FR9
