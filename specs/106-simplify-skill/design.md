...
The public surface is `automated simplify stage`. Normative behavior lives in `skills/simplify/` and follows the OMP extension, native `/plan`, and Herdr OMP worker boundaries defined by steering.
...
The maintained checks are `scripts/__tests__/simplify-contract.test.mjs`. They must exercise the public outcome rather than source-text coincidence.
