...
| AC5 | Pass | Closed path-annotation grammar accepts only Create, Modify, Delete, Download untracked, Generate untracked, and the minimal current-spec/test non-operation allowlist. A declaration is excluded only when its complete annotation normalizes by intentional ASCII hyphen, whitespace, and case handling to exactly `delivery-owner only`; residual, comma, prefixed, or suffixed text fails with located `publication_scope_unproven`. |
...
- `cd scripts && npm test -- --runInBand __tests__/sdlc-safe-recoveries.test.mjs` — passed: one suite, 113 tests.
- `cd scripts && npm test -- --runInBand __tests__/sdlc-apply-review.test.mjs __tests__/sdlc-deliver.test.mjs` — passed: two consumer suites, 133 tests (17 apply-review, 116 deliver).
- `cd scripts && npm test -- --runInBand` — passed: 55 suites passed, one suite skipped; 1,305 tests passed and two skipped (1,307 total).
- `cd scripts && npm test -- --runInBand __tests__/sdlc-safe-recoveries.test.mjs -t "reports exact PathCast 18/12/6 scope"` — passed: one disposable real-Git PathCast-state copy exercise; 112 unrelated tests skipped by name filter.
...
- Active #390 structured scope inspection — passed with 13 tracked, zero untracked, 13 allowed, four read-only spec inputs, and four task records; exact `verification-report.md` `(delivery-owner only)` is absent from both `allowedPaths` and `readOnlyPaths`.
...
- **HIGH — open path-annotation operation grammar: remediated.** The parser no longer treats unknown parenthetical notes as task-Type fallback. Exact supported operations remain case-insensitive for existing canonical lowercase declarations; all unsupported operation-bearing notes and ambiguous combinations fail closed with task/file/line diagnostics. `delivery-owner only` is excluded only when the complete annotation exactly matches after intentional ASCII hyphen, whitespace, and case normalization. Residual examples `(delivery-owner only, Archive)`, `(delivery-owner only Create)`, and `(Archive delivery-owner only)` enter the closed grammar and fail located `publication_scope_unproven`. Current-spec/test notes remain an explicit finite allowlist.
...
The final production and regression tree passed the safe-recoveries suite, focused consumers, and full Jest run listed above: 113 safe-recoveries tests; 17 apply-review and 116 delivery tests (133 focused consumer tests); 55 suites and 1,305 tests in the full run, with one suite and two tests skipped. Seven new safe-recoveries cases prove three accepted exact-normalization forms, three residual-text failures through both the public flat parser and the structured located-diagnostic parser, and exact `verification-report.md` exclusion from both implementation `allowedPaths` and `readOnlyPaths`. The amended supervisor fixture succeeds only after dirtying its canonical task-authorized `src/code.mjs`; its subjectless and wrong-issue cases still fail `publication_subject_unproven`. The unchanged extension and rendered-prompt contracts pass through accurate explicitly obsolete workflow wording, not test edits. Apply-review and delivery each retain a consumer-level regression that presents an `Archive` path annotation and observes `publication_scope_unproven` before commit, push, or merge-tree execution.
