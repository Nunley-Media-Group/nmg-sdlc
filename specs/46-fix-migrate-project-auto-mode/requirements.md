...
**Author**: Codex
**Severity**: High
**Related Spec**: `specs/25-migrate-project-skill/`
...
2. Add `.codex/unattended-mode` to the project directory
...
| **Configuration** | `.codex/unattended-mode` file present in project directory |

### Frequency

Always — whenever `.codex/unattended-mode` exists in the project directory.
...
| | Description |
|---|-------------|
| **Expected** | `/migrate-project` always presents proposed changes for interactive review via `interactive prompt` before modifying any files, regardless of whether `.codex/unattended-mode` exists |
| **Actual** | When `.codex/unattended-mode` exists, the skill skips the interactive review gate (AC4 from issue #25) and applies all changes without user approval |
...
**Given** a project with `.codex/unattended-mode` present and outdated steering docs
**When** `/migrate-project` runs and identifies changes to merge
**Then** proposed changes are presented for review via `interactive prompt` before any files are modified

**Example**:
- Given: A project with `.codex/unattended-mode` and a `product.md` missing the `## Product Principles` section
...
**Given** a project without `.codex/unattended-mode` and outdated steering docs
**When** `/migrate-project` runs and identifies changes to merge
**Then** proposed changes are presented for review via `interactive prompt` before any files are modified (existing behavior preserved)

### AC3: No Regression — Other Skills Still Respect unattended-mode

**Given** a project with `.codex/unattended-mode` present
...
|----|-------------|----------|
| FR1 | The `/migrate-project` skill MUST always use `interactive prompt` for the review gate, regardless of `.codex/unattended-mode` | Must |
| FR2 | The skill's prompt text must explicitly instruct Codex to ignore unattended-mode for this skill's review gate | Must |
