...
**GPT-5.5 rate-limit mitigation** — prior spec `specs/111-model-rate-limits/` established that concentrating GPT-5.5 use triggers rate limits mid-cycle. The new default config uses GPT-5.5 on only three of nine steps (down from a potential nine-of-nine under the old `"gpt-5.5"` global default). Mechanical steps (`startCycle`, `commitPush`, `merge`) drop to GPT-5.4 Mini — OpenAI's recommended model for deterministic tool-driven work — eliminating ~40% of previous GPT-5.5-subprocess starts per cycle.
