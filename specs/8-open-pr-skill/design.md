# Design: Open and Merge Pull Request
...
The public surface is `/sdlc-open-pr #N`. Normative behavior lives in `skills/open-pr/; skills/address-pr-comments/` and follows the OMP extension, native `/plan`, and Herdr OMP worker boundaries defined by steering.
...
The maintained checks are `scripts/__tests__/open-pr-delivery-contract.test.mjs; scripts/__tests__/sdlc-commands.test.mjs`. They must exercise the public outcome rather than source-text coincidence.
