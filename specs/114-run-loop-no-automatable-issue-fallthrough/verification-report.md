...
|----------|----------|----------|-------|-----|---------|
| Medium | Defect | scripts/sdlc-runner.mjs, scripts/__tests__/sdlc-runner.test.mjs | The runner can continue after issue selection returns no automatable issue, using stale or empty state. | Implemented terminal empty-queue handling. Added no-spawn regression coverage. Verified through the full scripts test suite. | direct / skill-creator-guided for skill-bundled docs |
