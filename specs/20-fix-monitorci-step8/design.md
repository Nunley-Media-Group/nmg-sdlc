...
**Primary cause**: The Step 8 prompt (line 546) ends with "Report the final CI status" — which Codex satisfies by reporting the failure and exiting cleanly with code 0. Compare Step 9's prompt (line 548) which explicitly says "do NOT merge — report the failure and exit with a non-zero status." Step 8 provides no equivalent non-zero exit instruction for unresolved failures.

**Secondary cause**: There is no post-step CI validation gate. After Step 3, the runner validates that all spec artifacts exist (lines 964-978) — even when Codex exits with code 0. Step 8 has no equivalent gate, so when Codex exits with code 0 after merely reporting CI status, the runner blindly advances to Step 9.
...
- Codex interprets "Report the final CI status" as a valid exit action after observing the failure
...
1. **Rewrite the Step 8 prompt** to give Codex explicit instructions: read CI logs, diagnose failures, apply fixes that don't deviate from specs, commit, push, re-poll. Exit non-zero if CI cannot be fixed without spec changes. Remove the "report and exit" escape.
...
- **Direct impact**: Step 8 prompt behavior (what Codex does during monitorCI) and post-Step-8 validation logic
...
|------|------------|------------|
| New prompt causes Codex to over-fix (change spec-specified behavior) | Low | Prompt explicitly instructs Codex to check specs before applying fixes; exit non-zero if fix requires spec deviation |
| CI validation gate false-positives on pending/queued checks | Low | Gate only triggers when Step 8 exits code 0 — Codex should have already polled to completion. Use same `/fail/i` check as Step 9 precondition. |
| Increased maxTurns/timeoutMin waste resources on unfixable issues | Low | Non-zero exit instruction means Codex escalates unfixable issues rather than spinning. Validation gate also caps retries at MAX_RETRIES. |
...
| Only add validation gate, keep prompt | Add the gate but don't change the prompt | Codex would still exit without fixing, just retry more. The gate alone doesn't teach Codex to actually fix CI. |
