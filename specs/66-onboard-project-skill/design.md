...
The public surface is `/sdlc-onboard-project`. Normative behavior lives in `skills/onboard-project/` and follows the OMP extension, native `/plan`, and Herdr OMP worker boundaries defined by steering.
...
The maintained checks are `scripts/__tests__/exercise-contribution-gate.test.mjs; scripts/__tests__/steering-contract.test.mjs`. They must exercise the public outcome rather than source-text coincidence.
