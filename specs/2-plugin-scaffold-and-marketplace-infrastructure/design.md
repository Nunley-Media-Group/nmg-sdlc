...
The public surface is `OMP extension load`. Normative behavior lives in `src/; commands/; package.json` and follows the OMP extension, native `/plan`, and Herdr OMP worker boundaries defined by steering.
...
The maintained checks are `scripts/__tests__/extension-commands.test.mjs; scripts/__tests__/plugin-surface-verification.test.mjs`. They must exercise the public outcome rather than source-text coincidence.
