# Design: Contribution Evidence Gate
...
The public surface is `managed pull-request workflow`. Normative behavior lives in `.github/workflows/nmg-sdlc-contribution-gate.yml; references/contribution-gate.md; CONTRIBUTING.md` and follows the OMP extension, native `/plan`, and Herdr OMP worker boundaries defined by steering.
...
The maintained checks are `scripts/__tests__/contribution-gate-contract.test.mjs; scripts/__tests__/exercise-contribution-gate.test.mjs`. They must exercise the public outcome rather than source-text coincidence.
