...
The public surface is `/sdlc-draft-issue`. Normative behavior lives in `skills/draft-issue/` and follows the OMP extension, native `/plan`, and Herdr OMP worker boundaries defined by steering.
...
The maintained checks are `scripts/__tests__/exercise-issue-form.test.mjs`. They must exercise the public outcome rather than source-text coincidence.
