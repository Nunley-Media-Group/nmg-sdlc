...
| `specs/107-add-skill-to-run-full-sdlc-pipeline-loop-from-within-codex/` | historical run-loop spec | Contains older text that says loop mode moves to the next issue after escalation; update it so future verification follows the corrected failure contract. |
...
| `specs/107-add-skill-to-run-full-sdlc-pipeline-loop-from-within-codex/` | Update older scenario/design text that said loop mode advances after escalation. | Keeps canonical specs from contradicting the corrected defect behavior. |
