...
**And** `grep -r -i 'discord' specs/` returns zero matches in spec bodies (not including the `specs/114-fix-stale-spec-references/` directory itself, which documents this very cleanup)
