...
- [ ] Step 4d (consolidation approval, lines 169–179) has unattended-mode guard: when `.claude/unattended-mode` exists, skip `AskUserQuestion`, record each consolidation group as a skipped operation, and proceed to Step 4f
- [ ] Step 9 Part A (steering doc approval, lines 336–356) has unattended-mode guard: when `.claude/unattended-mode` exists, auto-select all proposed sections without calling `AskUserQuestion`
- [ ] Step 9 Part B (batch approval, lines 358–379) has unattended-mode guard: when `.claude/unattended-mode` exists, auto-approve all non-destructive changes without calling `AskUserQuestion`; skip any remaining destructive operations with summary
...
- [ ] Step 10 "Persist declined sections" sub-step has unattended-mode guard: skip writing to `.claude/migration-exclusions.json` in unattended-mode (nothing is declined)
- [ ] No changes to Steps 1–3, 4a, 4b, 4c, 4e, 4f, 5, 6, 7, 8 analysis logic — only the approval gates and output are affected
- [ ] Interactive mode behavior (when `.claude/unattended-mode` does NOT exist) is completely unchanged

**Notes**: Follow the pattern from other skills — check for `.claude/unattended-mode` at each `AskUserQuestion` call site. The analysis steps (1–8) remain unchanged; only the approval steps (4d, 9) and output step (10) need unattended-mode branches. Keep changes minimal and focused on the three `AskUserQuestion` touchpoints plus the output section.
...
**Depends**: T001
**Acceptance**:
- [ ] Gherkin scenario reproduces the original bug condition (unattended-mode present, skill calls `AskUserQuestion`)
...
- [ ] Interactive mode flow (all `AskUserQuestion` calls) is preserved when `.claude/unattended-mode` does not exist
