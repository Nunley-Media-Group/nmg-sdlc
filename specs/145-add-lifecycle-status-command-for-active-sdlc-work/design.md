...
The public surface is `/sdlc-status [--json]`. Normative behavior lives in `skills/status/; scripts/sdlc-status.mjs` and follows the OMP extension, native `/plan`, and Herdr OMP worker boundaries defined by steering.
...
The maintained checks are `scripts/__tests__/sdlc-status.test.mjs; scripts/__tests__/status-skill-contract.test.mjs`. They must exercise the public outcome rather than source-text coincidence.
