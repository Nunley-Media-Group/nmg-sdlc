...
The public surface is `automated implementation stage`. Normative behavior lives in `skills/write-code/; agents/spec-implementer.md` and follows the OMP extension, native `/plan`, and Herdr OMP worker boundaries defined by steering.
...
The maintained checks are `scripts/__tests__/sdlc-execute.test.mjs`. They must exercise the public outcome rather than source-text coincidence.
