...
Approved specs #194 and #195 still require implement to keep simplify. This issue supersedes that contract after those issues merge. `specs/106-simplify-skill/` remains the historical simplify capability record and stays in the working-tree archive. Repository default branch is `main`. There is no nmg-sdlc `/review` command; review panes must submit host OMP `/review` directly to the existing worker TUI, choose PR-style review, and select literal `main`. They must not ask the worker model to launch a nested `omp` process. Review findings must not fail the review handoff, or the dedicated fix pane never runs.
...
**And** the implement worker commits all implementation changes with a conventional commit and pushes the issue branch before `review1` starts
...
**And** the controller sends Enter for PR-style mode, observes the distinct `Select base branch to compare against` menu, then sends the Down keys and final Enter for literal `main` in a separate key submission
**And** resume recognizes Herdr agent state from legacy `state` fields and actual `agent_status` or `agentStatus` fields
**And** a matching retained worker in `idle` or `done` with a passed, non-intervention handoff is closed, completed, and advances to the next step
...
| FR2 | Each review pane runs host OMP `/review` interactively in the existing worker TUI, sends the PR-style Enter, observes the branch-selection menu, then separately selects literal `main` and persists that review result. | Must | Never burst both menu answers or launch a nested `omp` command. Findings do not fail the review handoff. |
...
| FR5 | Reuse existing execute pane launch, handoff validation, resume, notification, and close-vs-keep behavior for the new panes; parse Herdr `state`, `agent_status`, and `agentStatus` response shapes, and close matching retained idle/done workers only after a passed non-intervention handoff. | Must | Do not fork the handoff schema. |
...
| FR8 | The implement worker stages all non-runtime implementation changes, creates a conventional commit, and pushes the issue branch before writing a passed implement handoff. | Must | Commit or push failure writes a failed intervention handoff; review1 must not start. |
| FR9 | Every review1, review2, fix1, and fix2 handoff includes `schemaVersion`, `issue`, `step`, string `summary`, `status`, `intervention`, `reasonCode`, `artifacts`, and `next`, and passes `validateHandoff`. | Must | Never synthesize a passed handoff in execute. |
