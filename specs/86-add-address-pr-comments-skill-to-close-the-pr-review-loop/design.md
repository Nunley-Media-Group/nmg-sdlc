# Design: Address Pull Request Comments
...
The public surface is `automated review remediation stage`. Normative behavior lives in `skills/address-pr-comments/` and follows the OMP extension, native `/plan`, and Herdr OMP worker boundaries defined by steering.
...
The maintained checks are `scripts/__tests__/open-pr-delivery-contract.test.mjs`. They must exercise the public outcome rather than source-text coincidence.
