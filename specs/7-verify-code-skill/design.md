...
The public surface is `/sdlc-verify-code #N`. Normative behavior lives in `skills/verify-code/; agents/architecture-reviewer.md` and follows the OMP extension, native `/plan`, and Herdr OMP worker boundaries defined by steering.
...
The maintained checks are `scripts/__tests__/plugin-surface-verification.test.mjs`. They must exercise the public outcome rather than source-text coincidence.
