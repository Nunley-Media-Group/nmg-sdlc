...
- `snippets[]`: `{ id, path, consumers, slot, order, byteBound }`. Consumers and slots must be allowed by the prompt registry. Provenance, ordering, placeholders, hashes, and byte bounds use the shared renderer.
