...
A settled failed, non-intervention handoff can start a fresh `rN-step` remediation worker for `implement`, either review/fix stage, `verify`, or `deliver`. Only one repair runs at a time. Other stages stop after two completed same-step remediations without advancement. Verification instead checkpoints each attempt's head, changed paths, registered gate and report failure fingerprint, and prior approach. Distinct published project changes plus changed failure evidence, or a genuinely new gate measurement at the repaired head, reset its consecutive no-progress count; there is no fixed total-attempt cutoff while progress continues. Unchanged/repeated evidence stops without another dispatch. A passed remediation advances and clears the streak.
...
After an issue-owned repair publishes a clean new head, a prior Fail/Partial verification report and exact-head gate artifact are archived with digests before one owner-bound recheck of that exact old/new head pair. A new failing gate produces a truthful new-head report and can guide a materially different repair; the identical pair cannot be consumed twice. Passing gates still require acceptance review and normal finalization. Same-head external-only Incomplete rechecks retain their separate one-use authority. Never delete a stale report or replay a consumed controller dispatch to obtain another attempt.
