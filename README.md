...
An explicit different issue can proceed past a stopped prior issue only after independent terminal proof. The existing recorded `merge_failed` deliver path remains; a cancelled pre-delivery or other failed-deliver checkpoint requires one CLOSED issue, its unique MERGED closing PR with exact head SHA, issue-prefixed head branch and default base, checkpoint ancestry to that PR head, and merge-commit ancestry on the clean current default checkout. Squash merges need not contain the PR head on default. A live or foreign worker/lease, missing or changed strict handoff, recovery owner, standalone session, review/verification or historical recovery evidence, ambiguous PR relation, unsafe path, or dirty checkout blocks with the failed proof and a read-only inspection action. Conflicting persisted post-merge PR/head or session-delivery identity also blocks. Before removing only the active `run.json`, the controller archives byte-exact run, handoffs, owner ledger, implicated sessions and evidence with SHA-256 digests and a run-identified receipt under `.omp/sdlc/archive/delivered-failures/<runId>/`; original evidence and unrelated records remain untouched. It never redispatches the closed issue or forges a passed handoff. Bare recovery and unproven cases remain blocked; do not manually delete the pointer.
...
| `.omp/sdlc/archive/delivered-failures/<runId>/` | Original stopped checkpoint, strict handoffs, recovery-owner ledger, implicated standalone sessions and review/verification/history bytes, SHA-256 manifest and exact merged-delivery receipt; originals retained |
