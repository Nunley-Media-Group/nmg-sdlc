...
This writes `sdlc-config.json` to the project root, adds it to `.gitignore`, and installs `.github/workflows/nmg-sdlc-contribution-gate.yml` when the approved path is absent or already nmg-sdlc-managed. The config specifies the project path, plugin root, per-step timeouts, model/effort choices, and which nmg-sdlc skills to inject. Codex plugin installs may use a versioned `pluginRoot` under `~/.codex/plugins/cache/...`; after plugin upgrades, run `$nmg-sdlc:upgrade-project` to refresh stale cache roots instead of creating a temporary config. The generated workflow runs on pull requests, checks for issue/spec/steering/verification evidence, uses read-only GitHub token permissions, requires no repository secrets by default, and does not replace project CI or human review.
