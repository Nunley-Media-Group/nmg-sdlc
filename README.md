...
Workers are sibling Herdr `--kind omp` panes. Execute owns orchestration, not product edits in the main pane. Worker success is determined by validated handoffs, not terminal prose or an exit code alone. Reviews use separate file-assigned snapshots outside the checkout, host-enforced read-only tools, and invocation-bound append-only receipts. Only host-captured final assistant output supplies findings; terminal or tool text cannot substitute for a review result. Execute awaits every review sibling before inspecting receipts, then rechecks each exact-assignment host JSONL up to 30 one-second observation pauses when the final result is not yet durable. It closes review panes only after all nonempty, uncontaminated results validate. Missing, malformed, foreign, contaminated, or empty evidence fails without a review handoff, closes no remaining review panes, and leaves assignments and receipts for diagnosis.
