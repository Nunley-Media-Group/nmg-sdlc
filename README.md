...
Owned worker panes close by default on successful completion, cancellation, and terminal non-debug stops. Cancellation reloads subordinate checkpoint writes and persists `controller_cancelled` plus the final worker disposition before releasing the controller lease; a persistence failure keeps cleanup fail-closed. `/sdlc-execute --retain-worker [#N …]` is the explicit debugging escape; `/sdlc-execute --recover-stale [#N …]` is the explicit post-preflight stale-lease recovery path and reclaims only the current run when the recorded PID is demonstrably absent and a successful Herdr listing proves the exact recorded controller pane is absent. Live or unknown PID/pane evidence, failed or unknown Herdr listings, and foreign run IDs preserve the lease and fail with `controller_lease_held`. Retained reuse requires the exact worker name, pane, project, run, issue, step, branch, and head recorded in the checkpoint. Prefix-colliding or identity-mismatched panes remain open and fail closed.
...
| sdlc-execute                 | /sdlc-execute [--retain-worker] [--recover-stale] [#N …] | Drive automated delivery through Herdr omp workers to merge + close |
