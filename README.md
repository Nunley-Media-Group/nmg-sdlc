...
A registered required applicable `project.*` provider can also route a failed deterministic local test to implementation when its exact-head, complete-coverage result explicitly contains `repairable: true` with structured command evidence (`program`, `args`, `cwd`, nonzero integer `exitCode`). The gate remains **Fail**. Missing/malformed signals, readiness failures, credentials, incomplete prerequisites and unrelated external failures remain intervention. An earlier five-field provider result has no marker: the operator may authorize **one** recovery against the original artifact bytes with bare `/sdlc-execute --legacy-recovery-digest=<64-hex-sha256>`. The controller checks issue/head/spec/run identity, failed handoff, ownership, branch, report-only dirtiness and unused recovery; it keeps the failed artifact/report, rewinds through implement, both reviews and all verification gates, and rejects changed bytes or repeated use. The digest authorizes remediation, not a passing result; never edit the retained artifact to add a flag.
