...
|---|---|
| `start` | Resolve the executable issue, establish its linked branch, safely reconcile an approved squash-merged spec branch with default, synchronize the exact head upstream, and bind that head in the controller before implement |
...
Workers are sibling Herdr `--kind omp` panes. Execute owns orchestration, not product edits in the main pane. Worker success is determined by validated handoffs, not terminal prose or an exit code alone. Reviews use separate file-assigned snapshots outside the checkout, host-enforced read-only tools, and invocation-bound append-only receipts. Only host-captured final assistant output supplies findings; terminal or tool text cannot substitute for a review result. Execute observes every review sibling until it is idle or done before reading any receipt; this worker-settlement loop has no deadline. It then rechecks each exact-assignment host JSONL for up to 30 one-second pauses when a final result is not yet durable. Missing, malformed, foreign, or empty evidence and worker failures retain all panes and receipts without a handoff or replacement. Proven prohibited access with complete host results retains the one-use, owner-bound whole-review replacement; a failed replacement stops without replenishment. Panes close only after all results for that attempt validate.
...
A settled failed, non-intervention handoff starts a fresh `rN-step` worker for `implement`, either review/fix stage, `verify`, or `deliver`. Only one repair runs at a time. The controller checkpoints each attempt's head, changed paths, exact report/gate or stage artifact digests, and prior approach; the next LLM worker reads the history and explains a different testable repair against the whole Approved spec. A newly published project change plus a fresh stage result, or distinct authorized review/PR-check evidence, resets the consecutive no-progress count. There is no fixed total attempt limit while new evidence exists. Unchanged inputs/results, ambiguous ownership, or a consumed side effect stop without another dispatch. Stage-specific publication, review, provider and exact-head delivery gates never become optional.

On the exact incomplete branch, bare `/sdlc-execute` can consume **one additional durable recovery allowance per run, issue and stopped stage**, including legacy checkpoints with 13 attempts. Consumption is persisted before dispatch and preserves the original history. A failed recovery, ambiguous dispatch, cancellation or process loss cannot authorize another repair from the same evidence. Repeated commands, commits, summary changes and plugin upgrades never replenish it. A genuinely validated passed handoff still settles; a different exact-head repair or registered gate result needs its own owner-bound proof and distinct one-use identity before another attempt.
...
Safe automatic recovery has a separate durable allowance for each class, logical owner, issue, and stage. Known committed publication is reconciled before another push; proven review contamination permits one whole-step replacement while preserving original evidence. Safe base reconciliation reruns every review/fix/verification gate on the new head. Bot review remains distinct from human authority, and post-merge observation never replays merge or closes an unrelated issue. Mere commits, leases, sessions or plugin versions do not replenish a consumed allowance; a new published stage artifact or gate result must pass its own exact owner-bound proof and consume its own one-use identity.

After an issue-owned source repair publishes a new head, a prior Fail/Partial verification report—or a mixed Incomplete report with a required local command failure—and its exact-head gate artifact are archived with digests before one owner-bound recheck of that exact old/new pair. A markerless legacy report also requires its exact issue comment before the server-observed repair push; unavailable push history fails closed. Every Pass, including standalone, requires a registered gate artifact with required results, true applicability, exact invocation, provider evidence and a source-head acceptance report. A known report-only commit can reconcile against its unchanged source parent, never a new source change. A truthful failed new-head report instead checkpoints its findings and rewinds to implementation; the LLM repairs source and both reviews plus verification rerun. Mixed external incompleteness never permits a passed handoff. Exact report/artifact snapshots are retained across rewinds, and the end goal stays every Approved criterion and required gate. Same-head external-only Incomplete retains its separate one-use authority. A stopped consumed verify loop may dispatch its distinct already-published repair only through exact-head recovery proof, never by clearing a checkpoint.
...
A failed `branch_checkout_failed` START may resume once after another worktree releases the issue branch. Bare execute must re-prove the original clean checkpoint, strict failed handoff, open issue, unique owner and absent worker, then archive and consume one recovery before the ordinary start worker. The worker never detaches or removes the other worktree. A reused branch with a squash-merged spec is reconciled by a verified non-force merge of approved default history and exact upstream synchronization; the passed handoff binds its branch/head to the controller checkpoint before implement.

If implement failed before publication because pre-existing reconciliation commits were ahead of upstream, synchronizing those exact commits does not by itself publish implementation. With an unchanged failed handoff, unique owner, exact approved history and synchronized head, no implementation commit, and only authorized dirty work, bare execute may consume one bounded recovery and run the normal implement bind/commit/push/handoff path. A changed head, ambiguous owner, different source, denied or unsafe ignored path, or consumed dispatch stays blocked. Never edit `run.json` to bypass the gate.

START/IMPLEMENT stop output reports the failed proof and observed branch, checkpoint/current/upstream heads or owning worktree when available. A still-owned branch requires its owner to release it; an exact old spec head still upstream requires only that pre-existing reconciliation head to be synchronized without force. After that external fact changes, a single `reprobe-once` choice checks the same run/issue and invokes bare execute only for a freshly proven, unconsumed recovery. Already-safe evidence resumes automatically without asking. Ambiguous ownership, different source/head, denied dirty work, and consumed dispatch report the missing proof and stay stopped; no generic retry or manual `run.json` rewrite.

Mature checkouts may retain historical `.omp/sdlc/sessions/` owner receipts and ignored local tool/configuration files. Recovery verifies session file shapes, size, no-follow identity, and owner binding; unrelated unknown runtime files or symlinks still block. An ignored ordinary file outside protected controller/spec/implementation paths is inert, never read as publication authority or staged by the exact dirty-path set. The exact ignored `.pi-glla/` directory is likewise inert only as a non-symlink directory outside implementation scope. Ignored implementation material, denied dirty paths, and ambiguous owner state do not become eligible.
...
An explicit `--recover-stale` invocation also authorizes the single owning-stage intervention when discovery returns a valid blocked recovery with that option. The owning public stage still runs once under its normal gates, then discovery runs once; invalid, incomplete, or ambiguous evidence remains stopped.
...
Blocked/intervention handoffs remain non-passing. Missing approval, unavailable credentials or required external evidence, unsafe ownership, and human-review authority are not permission to improvise success. An unsafe START/IMPLEMENT state prints the exact missing proof and stops without asking for a meaningless retry. Only a material owner-controlled release or synchronization offers a single re-probe prompt for that same issue/run. Other stages retain their checkpoint-bound intervention choices for a one-time owning standalone `verify` or `deliver` workflow or a read-only inspection; no prompt edits handoffs, grants another allowance, asks twice, or replays unchanged execution.
