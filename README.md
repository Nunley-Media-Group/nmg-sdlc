...
$nmg-sdlc:draft-issue
  ...
  → $nmg-sdlc:open-pr (review, exact-head merge, issue closure)
```
...
`$nmg-sdlc:address-pr-comments` remains available as a focused review-loop utility. `$nmg-sdlc:open-pr` uses the same review-remediation contract and does not report successful delivery until the exact verified head is merged and its issue is closed.
...
Epics are coordination-only and never own an executable feature package. Their aggregate package contains only cross-child outcomes and topology:
...
With an issue number, validates readiness, confirms the start, creates a linked branch, and updates project status. Without a number, presents milestones and ready issues for explicit selection. Coordination epics are removed before candidate ranking and can never be started, even by explicit issue number; an explicit epic request returns its ready and blocked children without mutating the epic. Each child is evaluated by the normal dependency and deliverable graph rules used for an ordinary issue. Confirmed epic membership is shown only as informational lineage, including nested ancestors, and is removed only from execution in-degree; genuine sibling and external prerequisites remain blockers. New epics persist an `epic` parent label, a matching `epic-child-of-N` child label, the native relationship, and body fallback so every fresh lifecycle command reconstructs the same identity. A structured `Requires deliverable` record is ready only when its matching whole-issue dependency has a closing pull request merged into the live default branch; issue closure alone is insufficient.

Automatic discovery expands its bounded candidate window after dependency filtering until the presentation target is filled or the cap is reached. It separately excludes open issues whose readable Project statuses are all `Done`; explicit issue input remains available with a warning before moving that work back to In Progress.
...
Creates human-reviewed requirements, technical design, tasks, stable-ID Gherkin scenarios, and feature issue-scope mappings for executable issues. Invoking it on an epic stops with a coordination-only explanation and its child readiness summary.

For the first ready child of an epic, the workflow reviews and publishes one three-file aggregate plus that child's separate normal package from a deterministic non-closing publication ref. Later children create or amend only their own packages and the matching aggregate manifest row. The aggregate owns cross-child outcomes and topology; the child owns every executable AC, FR, task, and scenario. Aggregate or sibling changes require a separate explicit review. Canonical default-branch proof is required before implementation, but an aggregate is not a prerequisite for starting the first child because that first child creates it.

Legacy sealed cumulative umbrella packages remain readable and repairable for compatibility. New child work never appends executable content to them; `$nmg-sdlc:upgrade-project` owns an explicitly approved split into aggregate and child authority.
...
Reads the active and relevant specs, resolves the active issue's delivery slice, presents a decision-complete implementation plan, waits for approval, executes only mapped owned-plus-adopted tasks sequentially, and runs simplification before reporting completion. An epic child must have valid aggregate/link authority, but aggregate outcomes remain bounded context rather than executable work. Invoking implementation on an epic stops without creating an empty plan. Resumption reconstructs progress inside the same child task set and ignores sibling or future work.
...
Inspects and stages only approved paths, requires matching active-scope verification, presents the version decision, updates version artifacts, commits, merges the refreshed base without rewriting published history, pushes, and creates or resumes one spec-linked pull request. Its acceptance criteria and test plan contain only mapped delivery plus separate declared regression evidence, link `issue-scope.json` when present, and use closing text only for the active executable issue—never an epic.

Opening the PR is progress, not completion. The skill fingerprints the exact head, checks, reviews, unresolved threads, merge state, and verification evidence on every observation. Pending checks are monitored; safe actionable findings are fixed, reverified, pushed, and observed again. A changed head invalidates prior evidence, and force-push is never used. Merge requires success-equivalent required checks, no blocking review or unresolved thread, exact-head verification, and live `mergeStateStatus: CLEAN`. The configured merge is performed only for that head. Success is reported only after the PR is `MERGED` and the child issue is `CLOSED`; otherwise the skill returns one evidence-backed external-authority blocker with its owner and recovery action.

After child closure, the workflow fully rehydrates its epic lineage. When every native direct child is closed and aggregate/child authority is valid, it reconciles readable Project status to Done, closes the eligible epic explicitly, verifies the result, and repeats leaf-to-root for nested ancestors. Partial pagination, zero children, cycles, open children, authority drift, or unreadable required Project state prevents automatic closure.
...
Status combines read-only git, normalized active issue scope, verification-report, issue, PR, body-marker, and check evidence. An epic reports `coordination-only`, its aggregate authority, children, lineage, and repair gaps without an executable next stage. An epic child resolves its exact linked package and reports aggregate lineage as informational context; planned, conflicting, or unverifiable authority routes to `write-spec` or `upgrade-project` instead of advancing. Delivery remains in progress until exact-head merge and issue closure proof. JSON mode emits `schemaVersion: 1` with stable lifecycle fields, nullable coordination/dependency results, and spec-scope plus epic-authority evidence. Status never prompts or mutates local or remote state.
...
The upgrade audit also reconciles epic identity and lifecycle state. Native sub-issue relationships are authoritative; supported body checklists are human-readable projections and drift evidence, never selection or closure authority. For each epic, the audit groups graph, aggregate/child authority, executable ownership, issue state, and readable Project state into one deterministic proposal with a digest over exact Git trees and live GitHub evidence. Audit is read-only.

Each epic mutation group requires its own explicit approval and a fresh identical pre-write digest. Eligible repairs include durable relationship/checklist correction, splitting a legacy cumulative package while moving only uniquely owned executable identifiers, Project Done/In Progress correction, stale-complete closure, premature-closure reopen, and nested reconciliation. Ambiguous prose or ownership is preserved and routed to an explicit missing-child or ownership decision. After application, the workflow rehydrates every approved target and requires a second audit to be a no-op; unrelated issues, specs, and Project items remain untouched.
...
| `$nmg-sdlc:start-issue [#N]` | Select and start ready executable work; epics return child readiness only |
| `$nmg-sdlc:write-spec #N` | Create child-owned executable specs and epic aggregate/link authority |
...
| `$nmg-sdlc:open-pr #N` | Version, deliver, remediate, exact-head merge, close the issue, and reconcile epics |
...
| `$nmg-sdlc:upgrade-project` | Reconcile contracts and apply exact approved, drift-checked epic/backlog repairs |
...
See [CONTRIBUTING.md](CONTRIBUTING.md) for the issue, epic, specification, verification, and terminal-delivery contracts used by this repository.
