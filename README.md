...
After an approved spec, `/sdlc-execute` drives automated SDLC delivery using Herdr `omp` worker panes for implementation (`write-code` plus behavior-preserving `simplify`), two deterministic review passes against the resolved GitHub default ref with dedicated finding-fix panes, verification, and delivery (`open-pr`). Implementation is conventionally committed and pushed before the first review. One exclusive `.omp/sdlc/controller.lock` lease protects the canonical project; competing execute, verification, or delivery controllers fail before mutation. Herdr/controller waits remain unbounded while the worker is observable; they end only on success, genuine failure, explicit cancellation, or confirmed process loss. Execute prefers the exact local default ref and otherwise accepts its exact `origin/` remote-tracking ref. Each controller-owned sibling `--kind omp` review receives one prompt that runs the PR-style merge-base host review and finalizes its canonical artifact and validated handoff. The controller waits on that evidence while the exact owned worker remains registered; it does not open or parse an interactive picker, require idle/working detection, start `--kind pi` workers, or stop Herdr.

Owned worker panes close by default on successful completion, cancellation, and terminal non-debug stops. Cancellation reloads subordinate checkpoint writes and persists `controller_cancelled` plus the final worker disposition before releasing the controller lease; a persistence failure keeps cleanup fail-closed. `/sdlc-execute --retain-worker [#N …]` is the explicit debugging escape; retained reuse requires the exact worker name, pane, project, run, issue, step, branch, and head recorded in the checkpoint. Prefix-colliding or identity-mismatched panes remain open and fail closed.
...
Implementation workers investigate missing in-scope components and implementation details, repair within approved constraints, and rerun verification before escalating. Remaining repairable work is a failed non-intervention handoff: execute preserves partial work and evidence for fresh `rN-implement` sessions under the original `implement` contract. Missing approved scope, unavailable required credentials or external evidence, conflicting safety authority, and unproven publication remain explicit intervention blockers. Neither repair nor remediation bypasses verification or the commit/push/clean-tree/upstream-equality gate before review1.
