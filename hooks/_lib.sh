#!/bin/bash
# Shared library for Claude Code hook scripts (OpenClaw notifications).
...
# gather_context — populate HOST, PROJECT, BRANCH_INFO, SESSION_NAME,
# COST, MSG_COUNT, LAST_PROMPT from git state and transcript.
gather_context() {
  HOST=$(hostname -s)
...
# build_message PREFIX — assemble notification string.
# Usage: build_message "CC finished"  or  build_message "CC waiting"
build_message() {
  local prefix="$1"
  MSG="${prefix} on ${HOST} | ${PROJECT}"
...
# send_claw_message CRON_PREFIX TEXT — deliver via one-shot openclaw cron job.
# Backgrounded with nohup because openclaw CLI takes ~5-10s to load plugins.
send_claw_message() {
  local cron_prefix="$1"
  local text="$2"
  nohup openclaw cron add \
    --at "1s" \
    --session isolated \
    --message "CC callback: $text — Acknowledge briefly." \
    --announce \
    --channel discord \
    --to "channel:${DISCORD_CHANNEL}" \
    --delete-after-run \
    --name "${cron_prefix}-$(date +%s)" \
    >/dev/null 2>&1 &
}
