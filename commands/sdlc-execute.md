...
1. Validate that `intervention.id` is `blocked-recovery`, `maxAttempts` is `1`, `options` contains exactly two distinct ids, and every non-null command is one of `/sdlc-status --json`, `/sdlc-verify-code #<intervention.issue>`, or `/sdlc-open-pr #<intervention.issue>` for the reported stage. Invalid intervention data is unreadable recovery evidence: print it and stop.
...
5. `run-owning-stage-once`: require `verify` with `/sdlc-verify-code #N` or `deliver` with `/sdlc-open-pr #N`. Read that workflow's complete `WORKFLOW.md` from this package and execute it inline exactly once for the reported issue under all of its normal ownership, approval, exact-head, and fail-closed gates.
...
- For `resumable`, `loop-recovery-available`, `consumed-dispatch-available` or `recovery-consumed`, invoke `node "<plugin-root>/scripts/sdlc-execute.mjs" run` once without issue tokens, forwarding only flags already supplied by the user. The controller revalidates identity and ownership; `consumed-dispatch-available` resumes only the exact pending, stopped, compatibility pane-split, or controller-loss-orphaned started consumed invocation without another allowance or recovery tuple, while other consumed unsuccessful recoveries remain non-replayable. Do not ask for a token, reason, flag or confirmation. Pass the result through and stop. If that invocation exits nonzero and one blocked intervention has not already been offered in this top-level invocation, rediscover once and use the parent fallback only when it returns a valid `blocked` intervention.
