...
7. Invoke `node <plugin-root>/scripts/sdlc-execute.mjs run` once with the selected numbers as separate `#N` tokens in the resolved order, followed by `--retain-worker` when it was present in `$ARGUMENTS`.
