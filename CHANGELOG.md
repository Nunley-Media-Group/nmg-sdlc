...
- Issue #195 moves versioning, PR create/resume, required-check and GraphQL review-thread polling, exact-head merge, refreshed closure proof, post-proof branch deletion, and deliver handoff ownership into `scripts/sdlc-deliver.mjs`; the extension resolves that controller from the installed package root, and the deliver worker receives remediation prompt weight only for actionable bot threads or failing required checks while terminating unactionable remediation as `human_review`.
