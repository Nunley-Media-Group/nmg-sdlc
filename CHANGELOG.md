...
- Resume an exact consumed repaired-publication dispatch after a pre-fix pane split or post-consumption agent-start/process loss without minting another allowance: preflight the standard worker pane from actual controller geometry before first consumption, persist invocation-bound pending/start dispositions by checkpoint CAS, require immutable archive and full owner/run/branch/handoff/task/worktree proof, and fail closed on mismatches, duplicate or ambiguous starts, explicit selectors, or unproven pane cleanup (#394).
...
- Add explicit recovery for confirmed stale controller leases (#328)
...
- Keep observing live review workers after prompt-wait failure (#320)
...
- Issue #284 resolves missing aggregate-check events from exact-head GitHub Actions runs and canonicalizes PR-scoped provenance without trusting empty fields.
...
- Issue #280 distinguishes complete zero-validation steering artifacts from missing, duplicate, or unknown declared results without weakening fail-closed verification.
...
- Replace Markdown steering with a managed runtime, /sdlc-steering, and deterministic verification (#214)
