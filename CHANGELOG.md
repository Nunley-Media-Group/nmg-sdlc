...
- `/sdlc-write-spec #N` now publishes an Approved `specs/{N}-{slug}/` package on a branch cut from the repository default, commits `docs: approve spec for #N`, pushes, squash-merges a docs-only spec PR (no `Closes #N`), then asks Continue/Finished. `/sdlc-execute` start-issue cuts implementation from that same default branch. Public commands inline bundled workflow files from `skills/` instead of emitting `/skill:`.
