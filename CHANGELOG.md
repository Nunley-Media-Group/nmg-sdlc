...
- `/sdlc-write-spec #N` now publishes an Approved `specs/{N}-{slug}/` package on `{N}-{slug}`, commits `docs: approve spec for #N`, pushes, then asks Continue/Finished. Public commands inline bundled workflow files from `skills/` instead of emitting `/skill:`.
