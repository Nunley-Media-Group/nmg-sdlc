...
- Allow parameter-free execute to recover one pre-implementation intervention only after owner-bound scope, exact HEAD/branch/run/owner identity, detector-selected byte-exact `Files` to `File(s)` repair, clean implementation paths, bounded terminal workflow evidence, and unused durable ownership are all proven; revalidate under the controller lease, preserve the failed handoff in history, and consume the implement-only recovery once (#392).
