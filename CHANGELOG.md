...
- Settle a resumed consumed-dispatch worker's fresh validated `implementation_failed` intervention after successful owned-pane cleanup so parameter-free discovery uses ordinary intervention classification without replaying the consumed invocation, minting authority, or changing immutable recovery evidence (#395).
