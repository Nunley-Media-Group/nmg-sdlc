...
1. **Resolve active issue scope** — reuse the validated `scoped` or `implicit_single_issue` result from `write-code` Step 2. For an epic child, first require the same `valid` link/aggregate authority digest and use only the child package. Enumerate only `delivery.tasks`, including explicitly adopted tasks; never begin from the entire cumulative `tasks.md` or aggregate outcomes.
