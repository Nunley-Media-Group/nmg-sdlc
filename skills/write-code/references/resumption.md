...
4. **Continue the workflow from Step 5** using the normal execution path (inline by default, optional Codex `worker` delegation only when explicitly authorized). Re-run Step 4 only if the plan needs revision based on what is already built.
...
| Commits exist but none match a task in `tasks.md` | Warn and present a `request_user_input` gate with `Treat as fresh` / `Stop as corrupt`; any free-form `Other` answer is treated as a task-id mapping hint and the task scan is retried. |
