...
6. Contribute outcomes to the Step 5 Versioning section using the same two-line outcome block as Step 2G.3a.
...
3. **Synthesize all four files in memory** using the templates loaded in 2B.4:
   ...
   - **`design.md`** — fill overview, architecture, API/interface changes from PR diff + PR body + commit messages + current code + current source tree. Include an explicit `## Evidence Sources` section listing which of {issue body, PR body, PR diff, commit messages, current code, **current source tree**} contributed to each major section; the `current source tree` row is present for every reconciled spec (marked `N/A — source-backfill` for PR-based rows when the spec was produced in source-backfill mode). If the issue degraded per 3B.2, include a `## Known Gaps` section noting the missing PR.
