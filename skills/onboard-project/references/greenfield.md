...
1. Resolve the design URL: if `--design-url <url>` was passed, use it. Else (interactive only) `request_user_input`: `"Provide a Design URL? (optional — press Enter to skip)"`. In unattended mode without `--design-url`, skip 2G.1 entirely.
...
- Hard floor: 3 candidates. Hard ceiling: 7. If interview output yields more, present a top-7 cut via `request_user_input` (auto-cut in unattended mode, with the cut list logged for Step 5).
...
`request_user_input`: `[1] Approve and proceed`, `[2] Adjust (return to candidate generation)`, `[3] Proceed without DAG (seed standalone)`.
...
1. In interactive mode, `request_user_input` whether to run `/init-config` now for unattended-runner setup. Options: `[1] Yes — run /init-config now`, `[2] No — skip, I'll run it later`.
