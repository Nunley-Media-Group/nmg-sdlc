...
7. Handle the GitHub semantic result:
   ...
   - `publication_closed_umbrella` → render the exact marked PR and matching `ClosedEvent` evidence. Ask through `request_user_input` for approval to reopen that exact issue. Only the exact reopen approval permits `gh issue reopen N`; silence, another approval, or a general continuation does not. Rerun the helper and require `merged_safe` with `evidence.recovered = true` before continuing.
