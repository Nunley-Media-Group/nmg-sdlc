...
After plan approval the executor publishes (`prepare` → write Approved package → `commit-push` → `merge` into the default branch) then asks Continue/Finished. Continuation never calls `xd://propose` and never re-enters phase review gates.
