...
3. **Call `request_user_input` once per gap** (up to the cap). The menu shape depends on the gap type and the issue's classification:
...
| Sentinel absent, 1–3 gaps detected | Ask one `request_user_input` per gap; thread answers into the draft |
