...
When the cached sentinel is absent, prompt via interactive user prompt:
...
- Never call interactive user prompt. Do not offer a fallback prompt and do not block on any input — that would hang the runner.
