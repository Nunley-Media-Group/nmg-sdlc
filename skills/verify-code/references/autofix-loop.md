...
Safe local edits for clear findings only. Route skill-bundled exclusively via /skill:skill-creator (if loaded) else fail caller handoff with skill_creator_missing.

Detector:
- paths under skills/ (all subdirs), root references/, agents/*.md
- task/finding text mentions skill authoring

Probe: presence in the current loaded skills list (system reminder).
