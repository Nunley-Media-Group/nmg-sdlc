...
The version-bump logic — classification matrix, epic-child sibling-aware downgrade, artifact updates, CHANGELOG roll — lives in one place and is read from both `$nmg-sdlc:open-pr` (historically) and `$nmg-sdlc:commit-push` (after FR1). This reference exists so `commit-push`'s SKILL.md can point at the canonical procedure without duplicating it.
...
- `major_requested` override (set by `$nmg-sdlc:open-pr` Step 0; in commit-push context the `--major` flag is not available and this override is always false).
...
## Differences between `$nmg-sdlc:open-pr` and `$nmg-sdlc:commit-push` invocations

| Aspect | `$nmg-sdlc:open-pr` historical behaviour | `$nmg-sdlc:commit-push` behaviour |
|--------|----------------------------------|--------------------------|
| `--major` flag | Honours `$nmg-sdlc:open-pr #N --major` | Not applicable — `$nmg-sdlc:commit-push` has no argument surface |
...
- CI monitoring and PR creation — owned by `$nmg-sdlc:open-pr` and the runner's `monitorCI` step.
