...
| Checklist stale | Confirmed `epic` parent with one unambiguous approved coordination-child set; native query succeeded; parent has a recognizable `## Child Issues` section; proposed edit changes only supported checklist rows to match those native children | Rewrite only that section through a temporary body file |

Do not offer automatic repair for multiple epic parents, multiple/mismatched child labels, a different native parent, malformed bodies, missing target metadata, unrecognized checklist structure, checklist content without confirmed umbrella identity, or evidence that could also represent an ordinary dependency. Preserve those records as inconsistent, ambiguous, manual-only, or unverifiable.
