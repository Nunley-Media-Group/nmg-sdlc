---
name: setup-steering
description: "Set up or enhance project steering documents (product, tech, structure). Use when user says 'set up steering', 'initialize project', 'bootstrap project', 'create steering docs', 'update steering', 'how do I set up the project', or 'how to bootstrap nmg-sdlc'. Do NOT use for writing specs or migrating existing projects to new templates. Bootstraps from templates on first run, enhances existing files in-place on subsequent runs. Prerequisite for the SDLC pipeline — run once before /draft-issue."
...
nmg-sdlc skills (/write-spec, /write-code, /verify-code).
...
Next step: Once steering docs are customized, run `/draft-issue` to create your first GitHub issue, then proceed through the SDLC pipeline.
```
...
```
/setup-steering (bootstrap or enhance)
         ↓
