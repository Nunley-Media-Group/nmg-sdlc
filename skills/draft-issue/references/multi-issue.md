...
3. Classify each reference as a whole-issue wait or an extracted baseline. Recommend a whole-issue wait unless the baseline is independently reviewable and parallel delivery materially requires another issue/PR.
4. For a whole-issue wait, add the owner-to-consumer DAG edge and store `{owner, consumer, description, boundary: "whole-issue"}` in the current scope's deliverable prerequisite list. For extraction, revise the proposed split so the baseline is its own ask, assign it ownership, and make every consumer depend on that ask.
