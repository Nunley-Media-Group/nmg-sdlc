...
**Unattended-mode rule.** `/draft-issue` as a whole does not honor unattended-mode (the skill header states this). The Epic child-creation flow is therefore always interactive — each child pass through Steps 2–9 may prompt as needed. Nothing about this sub-step introduces a new `request_user_input` call site beyond those already present in the Per-Issue Loop.
