...
Steering-level verification gates let a project declare mandatory shell-command checks that run as part of every `/sdlc-verify-code` pass — build-clean tests, security scanners, linters, skill-inventory audits, anything else the team wants enforced without baking it into the WORKFLOW.md. The workflow itself stays stack-agnostic: it only knows how to read gate rows from `steering/tech.md`, evaluate their conditions, run their Actions, and grade their Pass Criteria.
...
- **If absent**: no gates are enforced. This is backward-compatible — projects without the section run through `/sdlc-verify-code` unchanged.
