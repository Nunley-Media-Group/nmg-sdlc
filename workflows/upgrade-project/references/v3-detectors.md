...
Use the selected contract only for exactly one explicitly authorized issue package. Each invocation requires one publication command token and one `--spec specs/N-slug`; zero, repeated, or multiple specs and all ambiguous syntax fail before mutation. The package must contain four regular Approved files whose exact issue digit strings match its directory. Caller root, specs root, package, and inventory must not traverse symlinks.

Detection hashes canonical root/package, complete directly sorted inventory with digests/lstat identities, and exact rewrite/finding plan. Apply creates one exclusive fsynced regular root lock containing token/pid bytes and one exclusive fsynced root stage. `O_NOFOLLOW` is used where available; portable proof also requires matching regular lstat identity before open, matching descriptor identity, exact bytes, and matching lstat after read. Under the lock it reruns detection, verifies target/stage, then atomically renames once. Rename failure leaves the original untouched. Pre-existing, symlinked, partial, wrong-byte, or replaced locks are retained. Post-rename unlink failure reports `publication_files_cleanup_failed`, `applied_cleanup_failed`, and `applied: true`.

`applyPublicationUpgrade` and `apply-publication` call only the publication line rewriter. Minimal approved ASCII Buffer slices preserve invalid UTF-8 and line endings; repeated detection returns `writeCount: 0`. They never call general apply, dependencies, label backfill, GitHub, or another phase.
