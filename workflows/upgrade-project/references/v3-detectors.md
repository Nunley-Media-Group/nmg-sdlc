...
Detection hashes canonical root/package, complete directly sorted inventory with digests/lstat identities, and exact rewrite/finding plan. Apply creates one exclusive fsynced regular root lock containing token/pid bytes and one exclusive fsynced root stage. Authorized nmg-sdlc invocations honor the lock. Under that cooperative model, apply reruns complete authority and validates the final target through pre-open lstat, one `O_RDONLY | O_NOFOLLOW` descriptor where supported, descriptor fstat and exact bytes, and post-read path identity; without `O_NOFOLLOW`, the lstat/fstat/post-lstat chain supplies the proof. It separately validates exact stage identity/bytes before one atomic rename. Every observed pre-boundary change fails closed. Successful rename defines `applied: true`; a later cleanup failure retains validated lock evidence.

Node has no portable identity-conditional pathname rename or unlink. A non-cooperative same-credential actor that deliberately replaces or mutates the lock, stage, or target after its last validation and inside `renameSync` or `unlinkSync` is undefined external interference outside this contract; no claim is made that such replacement during either syscall is impossible.
