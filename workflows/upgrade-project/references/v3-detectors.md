...
Use the selected contract whenever publication authorization names fewer packages than the repository contains. Each invocation requires exactly one publication command token and rejects unknown options, unexpected positionals, duplicate singleton options, missing or option-like values, and extra command tokens before mutation. Malformed or duplicate apply approvals retain `publication_files_approval_invalid`; unrelated publication syntax uses `publication_cli_invalid`. Legacy command-looking option values remain values. `--spec` is repeatable and mandatory. Each value must name a unique direct `specs/{N}-{slug}/` package with all four required regular files; every required file must declare matching singular `**Issue**: #N` and `**Status**: Approved`. The caller's raw root spelling is inspected before normalization; its final component and every lexically traversed ancestor—including a symlink before `..`—plus `specs/`, selected packages, and inventoried entries must not traverse symlinks.

Detection sorts the selection, inventories every selected regular file recursively, and hashes a report envelope containing the exact symlink-free caller root, exact selection, every file source digest and lstat identity (`device`, `inode`, `mode`, `size`), and exact publication rewrite/finding records. Approval is the returned `publication-files:<digest>`. Apply acquires a project-owned mutation lock, constructs outputs from exact Buffer snapshots, stages every output, then revalidates the complete selected inventory, target identities, and bytes immediately before commit. A stale report or same-byte target replacement fails before commit. A multi-target write or rename failure restores every target's original bytes and original inode where the saved original target is available. Cleanup requires the captured lock-directory device/inode and, once written, the matching owner token; self-owned metadata setup failure cannot strand an empty lock, and existing/unproven locks remain untouched.

`applyPublicationUpgrade` and `apply-publication` call only the publication line rewriter. They do not call `applyUpgrade`, issue-dependency mutation, `spec-created` label backfill, GitHub, or another detector/apply phase. Ordinary unambiguous ASCII label-plus-prose repair remains one compatible planned rewrite with its final `after`. When opaque bytes prevent safe payload mapping, only an exact label span may be planned and the payload remains a blocking finding. Apply splices minimal approved ASCII spans between untouched Buffer slices, preserving invalid UTF-8 and each existing line ending. Repeated selected detection returns `writeCount: 0`.
