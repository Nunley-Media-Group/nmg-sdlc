...
When authorization is bounded to named issue-owned packages, do not use repository-wide `detect` or `apply`. Run `detect-publication --root <real-project-root> --spec specs/N-slug` with one repeatable `--spec` per explicitly authorized package. Inspect the caller's raw root spelling before normalization: its final component and every lexically traversed ancestor, including a symlink before `..`, must be symlink-free. Present only that report's exact `publication-files:<digest>` item. Approved execution must run `apply-publication` with the same root, same complete `--spec` set, and exact approved id. This path applies only selected publication rewrites under a project-owned mutation lock, final complete-inventory revalidation, staged commit, and byte rollback. Lock cleanup requires captured directory identity and, after metadata creation, its ownership token. It preserves arbitrary unrelated bytes and never runs dependency repair, `spec-created` backfill, or another upgrade phase. Missing, duplicate, outside, symlinked, incomplete, non-Approved, issue-mismatched, stale, foreign, or ambiguous CLI inputs fail closed.
