...
When authorization is bounded to one issue-owned package, do not use repository-wide `detect` or `apply`. Run `detect-publication --root <real-project-root> --spec specs/N-slug`, then `apply-publication` with that same single spec and exact approved id. Zero, repeated, or multiple specs fail. Apply uses one exclusive fsynced root lock and one fsynced root stage. Regular-file lstat checks before and after open/read prove identity even where `O_NOFOLLOW` is unavailable. It reruns authority, verifies target/stage, and atomically renames once. Rename failure leaves the original untouched; failed lock unlink reports `applied: true` and retains evidence. Existing, symlinked, or replaced locks/stages are never mutated. No dependencies, backfill, GitHub, or other phase runs.
