...
- When `$ARGUMENTS` includes `--recover-stale`, a valid blocked intervention with a `run-owning-stage-once` option is executed inline exactly once without `ask`; after that attempt, rediscover exactly once and resume only from a validated result. A missing option, invalid intervention, or non-resumable rediscovery remains a stop.
