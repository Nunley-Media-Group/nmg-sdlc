...
1. Choose the exact implementation commit subject before staging. It must use `feat:`, `fix:`, `docs:`, or `chore:` (optionally with a conventional scope/breaking marker), describe the change, and contain the literal issue identifier `#N`. Run the same `bind` command again after tasks and simplification, adding `--subject "<exact planned subject>"`, to obtain the current approved `allowedPaths` and machine-check the subject. Invoke the helper with a program-and-argument array; never interpolate the subject into shell source. Require exit 0 and `NMG_SDLC_PUBLICATION` with `passed:true` before any staging, commit, or push. For a dirty non-runtime worktree, `bind` calls `assertInitialStagePublication` under the controller lease; a consumed publication allowance must stop this worker before Git mutations, even after additional approved edits or commit churn. Run `git status --porcelain=v1 -z` and collect every changed path except `.omp/` runtime state, including both sides of renames. Every changed path must belong to that helper-provided set; never widen scope from model assertions. If the tree was clean at binding but becomes dirty before staging, repeat this subject-bearing `bind` before Git mutation.
